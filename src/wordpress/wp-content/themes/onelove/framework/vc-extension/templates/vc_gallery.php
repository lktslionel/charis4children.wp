...
$columns = $with_spacing = '';
$groupID = catanis_random_string('5');
...
if ( 'nivo' === $type ) {
	...
	$slides_wrap_start = '<div class="nivoSlider cata-nivoSlider">';
	$slides_wrap_end = '</div>';
...
if ( 'flexslider' === $type || 'flexslider_fade' === $type || 'fading' === $type ) {
	$type = ' wpb_flexslider flexslider_fade flexslider cata-flexslider-fade';
	$flex_fx = ' data-flex_fx="fade"';
} elseif ( 'flexslider_slide' === $type ) {
	$type = ' wpb_flexslider flexslider_slide flexslider cata-flexslider-slide';
...
foreach ( $images as $i => $image ) {
	...
	switch ( $source ) {
		case 'media_library':
			if ( $image > 0 ) {
				...
				/*$attachment_title = '<span>'.get_the_title($image).'</span>';*/
			} else {
	...
	switch ( $onclick ) {
		...
		case 'link_image':
			if ( 'wpb_image_grid' === trim($type) ) {
				$link_start = '<a data-fresco-group="product-gallery_'. $groupID .'" class="fresco" href="' . $large_img_src . '">';
				$link_start .= '<div class="cata-overlay"><span class="icon-circle"><i class="ti-plus"></i></span></div>';
				$link_end = '</a>';
			}else{ 
				/*prettyphoto*/
				$link_start = '<a data-fresco-group="product-gallery_'. $groupID .'" class="fresco" href="' . $large_img_src . '">';
	...
	$gal_images .= $el_start . $link_start . $thumbnail . $link_end . $attachment_title . $el_end;
}

$class_to_filter = 'wpb_gallery wpb_content_element vc_clearfix cata-'. $columns . ' ' . $with_spacing;
