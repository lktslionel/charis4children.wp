...
/**
 * ============================================================
 * Description: Simple Breadcrumb for page, post, taxonomy,...
 *
 * @name		Catanis_Breadcrumb
...
if ( !function_exists( 'catanis_print_breadcrumbs' ) ) {
	function catanis_print_breadcrumbs( $args = array() ) {
		...
		<?php
			...
			$breadcrumb = new Catanis_Breadcrumb( $args );
			return $breadcrumb->trail();
		}
	}
}

if( !class_exists('Catanis_Breadcrumb') ){
	class Catanis_Breadcrumb {
		...
		public function __construct( $args = array() ) {
			...
			$defaults = array(
				...
				'after'           => '',
				'show_on_front'   => true,
				'home_label'      => esc_html__( 'Home', 'onelove' ),
				'show_title'      => true,
				'post_taxonomy'   => array(
					'post' => 'category',
				),
			...
			$args = apply_filters( 'catanis_breadcrumb_args', $args );
		...
		public function trail() {
			...
			if ( 0 < $item_count ) {
				...
				$breadcrumb .= '<ul class="cata-breadcrumb-items" itemscope="breadcrumb" itemtype="http://schema.org/BreadcrumbList">';
				...
				foreach ( $this->items as $item ) {
					...
					$item_class = 'cata-breadcrumb-item';
					if ( 1 === $item_position && 1 < $item_count ) {
						$item_class .= ' cata-breadcrumb-first';
					} elseif ( $item_count === $item_position ) {
						$item_class .= ' cata-breadcrumb-last';
					}
				...
				echo sprintf(
					'<%1$s id="cata-breadcrumbs" class="cata-breadcrumbs cata-list-divider">%2$s%3$s%4$s</%1$s>',
		...
		protected function add_items() {
			...
			if ( is_front_page() ) {
				...
				if ( is_home() ) {
					...
					if ( is_post_type_archive() ) {
						...
						$this->add_rewrite_front_items();
						$year  = sprintf( '%s',  get_the_time( esc_html__( 'Y',  'onelove' ) ) );
						$month = sprintf( '%s', get_the_time( esc_html__( 'F', 'onelove' ) ) );
						$day   = sprintf( '%s',   get_the_time( esc_html__( 'j', 'onelove' ) ) );
						...
						if ( true === $this->args['show_title'] ) {
							$this->items[] = esc_html__( 'Archives', 'onelove' );
						}
					}
					...
					if ( is_paged() ) {
						$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_search_link() ), sprintf( esc_html__( 'Search Results for : %s', 'onelove' ), get_search_query() ) );
					} elseif ( true === $this->args['show_title'] ) {
						$this->items[] = sprintf( esc_html__( 'Search Results for : %s', 'onelove' ), get_search_query() );
					}
					
				} elseif ( is_404() ) {
					if ( true === $this->args['show_title'] ) {
						$this->items[] = esc_html__( '404 Not Found', 'onelove' );
					}
				}
			}
			...
			if ( is_singular() && 1 < get_query_var( 'page' ) && true === $this->args['show_title'] ) {
				$this->items[] = sprintf( esc_html__( 'Page %s', 'onelove' ), number_format_i18n( absint( get_query_var( 'page' ) ) ) );
			} elseif ( is_paged() && true === $this->args['show_title'] ) {
				$this->items[] = sprintf( esc_html__( 'Page %s', 'onelove' ), number_format_i18n( absint( get_query_var( 'paged' ) ) ) );
			}
	
			/*Filter hook for items*/
			$this->items = array_unique( apply_filters( 'catanis_breadcrumb_items', $this->items, $this->args ) );
		}
			...
			if ( false !== $taxonomy->rewrite ) {
				...
				if ( $taxonomy->rewrite['slug'] ) {
					...
					/* Deals with the situation if the slug has a '/' between multiple
					 strings. For example, "movies/genres" where "movies" is the post type archive. */
		...
		protected function add_post_terms( $post_id, $taxonomy ) {
			...
			if ( $terms && ! is_wp_error( $terms ) ) {
				...
				usort( $terms, 'wp_list_sort' ); /*_usort_terms_by_ID*/
