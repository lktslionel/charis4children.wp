...
// Theme init
if (!function_exists('charity_is_hope_widget_recent_reviews_theme_setup')) {
	add_action( 'charity_is_hope_action_before_init_theme', 'charity_is_hope_widget_recent_reviews_theme_setup', 1 );
	function charity_is_hope_widget_recent_reviews_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('charity_is_hope_exists_visual_composer') && charity_is_hope_exists_visual_composer())
			add_action('charity_is_hope_action_shortcodes_list_vc','charity_is_hope_widget_recent_reviews_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('charity_is_hope_widget_recent_reviews_load')) {
	add_action( 'widgets_init', 'charity_is_hope_widget_recent_reviews_load' );
	function charity_is_hope_widget_recent_reviews_load() {
		register_widget('charity_is_hope_widget_recent_reviews');
	}
}

// Widget Class
class charity_is_hope_widget_recent_reviews extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_reviews', 'description' => esc_html__('Recent reviews', 'trx_utils'));
		parent::__construct( 'charity_is_hope_widget_recent_reviews', esc_html__('Charity Is Hope - Recent reviews', 'trx_utils'), $widget_ops );
...
*/
if ( !function_exists( 'charity_is_hope_sc_widget_recent_reviews' ) ) {
	function charity_is_hope_sc_widget_recent_reviews($atts, $content=null){	
		...
		extract($atts);
		$type = 'charity_is_hope_widget_recent_reviews';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, charity_is_hope_prepare_widgets_args(charity_is_hope_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_reviews', 'widget_recent_reviews') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('charity_is_hope_shortcode_output', $output, 'trx_widget_recent_reviews', $atts, $content);
	}
	charity_is_hope_require_shortcode("trx_widget_recent_reviews", "charity_is_hope_sc_widget_recent_reviews");
}


// Add [trx_widget_recent_reviews] in the VC shortcodes list
if (!function_exists('charity_is_hope_widget_recent_reviews_reg_shortcodes_vc')) {
	function charity_is_hope_widget_recent_reviews_reg_shortcodes_vc() {
