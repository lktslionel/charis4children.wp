...
// Theme init
if (!function_exists('charity_is_hope_widget_recent_posts_theme_setup')) {
	add_action( 'charity_is_hope_action_before_init_theme', 'charity_is_hope_widget_recent_posts_theme_setup', 1 );
	function charity_is_hope_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('charity_is_hope_exists_visual_composer') && charity_is_hope_exists_visual_composer())
			add_action('charity_is_hope_action_shortcodes_list_vc','charity_is_hope_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('charity_is_hope_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'charity_is_hope_widget_recent_posts_load' );
	function charity_is_hope_widget_recent_posts_load() {
		register_widget('charity_is_hope_widget_recent_posts');
	}
}

// Widget Class
class charity_is_hope_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'trx_utils'));
		parent::__construct( 'charity_is_hope_widget_recent_posts', esc_html__('Charity Is Hope - Recent Posts', 'trx_utils'), $widget_ops );
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'trx_utils')), charity_is_hope_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'charity_is_hope_sc_widget_recent_posts' ) ) {
	function charity_is_hope_sc_widget_recent_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'charity_is_hope_widget_recent_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_recent_posts' 
								. (charity_is_hope_exists_visual_composer() ? ' vc_widget_recent_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, charity_is_hope_prepare_widgets_args(charity_is_hope_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('charity_is_hope_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	charity_is_hope_require_shortcode("trx_widget_recent_posts", "charity_is_hope_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('charity_is_hope_widget_recent_posts_reg_shortcodes_vc')) {
	function charity_is_hope_widget_recent_posts_reg_shortcodes_vc() {
