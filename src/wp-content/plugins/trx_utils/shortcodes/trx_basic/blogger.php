...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('charity_is_hope_sc_blogger_theme_setup')) {
	add_action( 'charity_is_hope_action_before_init_theme', 'charity_is_hope_sc_blogger_theme_setup' );
	function charity_is_hope_sc_blogger_theme_setup() {
		add_action('charity_is_hope_action_shortcodes_list', 		'charity_is_hope_sc_blogger_reg_shortcodes');
		if (function_exists('charity_is_hope_exists_visual_composer') && charity_is_hope_exists_visual_composer())
			add_action('charity_is_hope_action_shortcodes_list_vc','charity_is_hope_sc_blogger_reg_shortcodes_vc');
	}
}
...
*/
charity_is_hope_storage_set('sc_blogger_busy', false);

if (!function_exists('charity_is_hope_sc_blogger')) {	
	function charity_is_hope_sc_blogger($atts, $content=null){	
		if (charity_is_hope_in_shortcode_blogger(true)) return '';
		extract(charity_is_hope_html_decode(shortcode_atts(array(
			...
			"location" => "default",
			"dir" => "horizontal",
			"hover" => charity_is_hope_get_theme_option('hover_style'),
			"hover_dir" => charity_is_hope_get_theme_option('hover_dir'),
			"scroll" => "no",
			"controls" => "no",
			"rating" => "no",
			"info" => "yes",
		...
		charity_is_hope_storage_set('sc_blogger_busy', true);
		charity_is_hope_storage_set('sc_blogger_counter', 0);
		...
		if ($descr == '') $descr = charity_is_hope_get_custom_option('post_excerpt_maxlength'.($columns > 1 ? '_masonry' : ''));
	
		if (!charity_is_hope_param_is_off($scroll)) {
			charity_is_hope_enqueue_slider();
			if (empty($id)) $id = 'sc_blogger_'.str_replace('.', '', mt_rand());
		}
		
		$class = apply_filters('charity_is_hope_filter_blog_class',
					'sc_blogger'
					. ' layout_'.esc_attr($style)
					. ' template_'.esc_attr(charity_is_hope_get_template_name($style))
					. (!empty($class) ? ' '.esc_attr($class) : '')
					. ' ' . esc_attr(charity_is_hope_get_template_property($style, 'container_classes'))
					. ' sc_blogger_' . ($dir=='vertical' ? 'vertical' : 'horizontal')
					. (charity_is_hope_param_is_on($scroll) && charity_is_hope_param_is_on($controls) ? ' sc_scroll_controls sc_scroll_controls_type_top sc_scroll_controls_'.esc_attr($dir) : '')
					. ($descr == 0 ? ' no_description' : ''),
					array('style'=>$style, 'dir'=>$dir, 'descr'=>$descr)
		);
	
		$container = apply_filters('charity_is_hope_filter_blog_container', charity_is_hope_get_template_property($style, 'container'), array('style'=>$style, 'dir'=>$dir));
		...
		$container2 = apply_filters('charity_is_hope_filter_blog_container2', charity_is_hope_get_template_property($style, 'container2'), array('style'=>$style, 'dir'=>$dir));
		...
		$output = '<div'
			...
			. '>'
			. ($container_start)
			. (!empty($subtitle) ? '<h6 class="sc_blogger_subtitle sc_item_subtitle">' . trim(charity_is_hope_strmacros($subtitle)) . '</h6>' : '')
			. (!empty($title) ? '<h2 class="sc_blogger_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(charity_is_hope_strmacros($title)) . '</h2>' : '')
			. (!empty($description) ? '<div class="sc_blogger_descr sc_item_descr">' . trim(charity_is_hope_strmacros($description)) . '</div>' : '')
			. ($container2_start)
			. ($style=='list' ? '<ul class="sc_list sc_list_style_iconed">' : '')
			. ($dir=='horizontal' && $columns > 1 && charity_is_hope_get_template_property($style, 'need_columns') ? '<div class="columns_wrap">' : '')
		...
		if (charity_is_hope_get_template_property($style, 'need_isotope')) {
			if (!charity_is_hope_param_is_off($filters))
				$output .= '<div class="isotope_filters"></div>';
			if ($columns<1) $columns = charity_is_hope_substr($style, -1);
		...
		if (!charity_is_hope_param_is_off($only)) $args = charity_is_hope_query_add_filters($args, $only);
		...
		while ( $query->have_posts() ) { $query->the_post();
			...
			charity_is_hope_storage_inc('sc_blogger_counter');
	
			$args = array(
				'layout' => $style,
				'show' => false,
				'number' => charity_is_hope_storage_get('sc_blogger_counter'),
				'add_view_more' => false,
				...
				"readmore" => $readmore,
				"loadmore" => $loadmore,
				"reviews" => charity_is_hope_param_is_on($rating),
				"dir" => $dir,
				"scroll" => charity_is_hope_param_is_on($scroll),
				"info" => charity_is_hope_param_is_on($info),
				...
				// Get post data
				'strip_teaser' => false,
				'content' => charity_is_hope_get_template_property($style, 'need_content'),
				'terms_list' => !charity_is_hope_param_is_off($filters) || charity_is_hope_get_template_property($style, 'need_terms'),
				'filters' => charity_is_hope_param_is_off($filters) ? '' : $filters,
				'hover' => $hover,
				'hover_dir' => $hover_dir
			);
		...
		if (!charity_is_hope_param_is_off($filters)) {
			...
			if ($filters == 'categories') {			// Use categories as filter items
				...
				$portfolio_parent = $cat ? max(0, charity_is_hope_get_parent_taxonomy_by_property($cat, 'show_filters', 'yes', true, $taxonomy)) : 0;
				$args2 = array(
					'type'			=> $post_type,
		...
		$output	.= (charity_is_hope_param_is_on($scroll) 
				? '</div></div><div id="'.esc_attr($id).'_scroll_bar" class="sc_scroll_bar sc_scroll_bar_'.esc_attr($dir).' '.esc_attr($id).'_scroll_bar"></div></div>'
					. (!charity_is_hope_param_is_off($controls) ? '<div class="sc_scroll_controls_wrap"><a class="sc_scroll_prev" href="#"></a><a class="sc_scroll_next" href="#"></a></div>' : '')
				: '')
			. ($dir=='horizontal' && $columns > 1 && charity_is_hope_get_template_property($style, 'need_columns') ? '</div>' :  '')
			. ($style == 'list' ? '</ul>' : '')
			. ($container2_end)
			. (!empty($link) 
		...
		return apply_filters('charity_is_hope_shortcode_output', $output, 'trx_blogger', $atts, $content);
	}
	charity_is_hope_require_shortcode('trx_blogger', 'charity_is_hope_sc_blogger');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'charity_is_hope_sc_blogger_reg_shortcodes' ) ) {
	//add_action('charity_is_hope_action_shortcodes_list', 'charity_is_hope_sc_blogger_reg_shortcodes');
	function charity_is_hope_sc_blogger_reg_shortcodes() {
	
		charity_is_hope_sc_map("trx_blogger", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"value" => "accordion",
					"type" => "select",
					"options" => charity_is_hope_get_sc_param('blogger_styles')
				),
				"filters" => array(
					...
					"dir" => "horizontal",
					"type" => "checklist",
					"options" => charity_is_hope_get_sc_param('filters')
				),
				"hover" => array(
					...
					"value" => "",
					"type" => "select",
					"options" => charity_is_hope_get_sc_param('hovers')
				),
				"hover_dir" => array(
					...
					"value" => "left_to_right",
					"type" => "select",
					"options" => charity_is_hope_get_sc_param('hovers_dir')
				),
				"dir" => array(
				...
				"post_type" => array(
					...
					"value" => "post",
					"type" => "select",
					"options" => charity_is_hope_get_sc_param('posts_types')
				),
				"ids" => array(
				...
				"only" => array(
					...
					"value" => "no",
					"type" => "select",
					"options" => charity_is_hope_get_sc_param('formats')
				),
				"scroll" => array(
				...
				"location" => array(
					...
					"value" => "default",
					"type" => "select",
					"options" => charity_is_hope_get_sc_param('locations')
				),
				"rating" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'charity_is_hope_sc_blogger_reg_shortcodes_vc' ) ) {
	//add_action('charity_is_hope_action_shortcodes_list_vc', 'charity_is_hope_sc_blogger_reg_shortcodes_vc');
	function charity_is_hope_sc_blogger_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"std" => "accordion",
					"class" => "",
					"value" => array_flip(charity_is_hope_get_sc_param('blogger_styles')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "filters",
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(charity_is_hope_get_sc_param('filters')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "hover",
					...
					"class" => "",
					"value" => array_flip(charity_is_hope_get_sc_param('hovers')),
					'dependency' => array(
						'element' => 'style',
				...
				array(
					...
					"class" => "",
					"value" => array_flip(charity_is_hope_get_sc_param('hovers_dir')),
					'dependency' => array(
						'element' => 'style',
						'value' => array('portfolio_2','portfolio_3','portfolio_4','grid_2','grid_3','grid_4','square_2','square_3','square_4','short_2','short_3','short_4','colored_2','colored_3','colored_4')
					),
				...
				array(
					...
					"value" => array_flip(charity_is_hope_get_sc_param('locations')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "dir",
				...
				array(
					...
					"class" => "",
					"value" => array_flip(charity_is_hope_get_sc_param('posts_types')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "ids",
				...
				array(
					...
					"class" => "",
					"group" => esc_html__('Query', 'trx_utils'),
					"value" => array_flip(charity_is_hope_get_sc_param('formats')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scroll",
		...
		class WPBakeryShortCode_Trx_Blogger extends CHARITY_IS_HOPE_VC_ShortCodeSingle {}
	}
}
?>