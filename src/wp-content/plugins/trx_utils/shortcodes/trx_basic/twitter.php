...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('charity_is_hope_sc_twitter_theme_setup')) {
	add_action( 'charity_is_hope_action_before_init_theme', 'charity_is_hope_sc_twitter_theme_setup' );
	function charity_is_hope_sc_twitter_theme_setup() {
		...
		if (function_exists('charity_is_hope_exists_visual_composer') && charity_is_hope_exists_visual_composer())
			add_action('charity_is_hope_action_shortcodes_list_vc','charity_is_hope_sc_twitter_reg_shortcodes_vc');
	}
}
...
if (!function_exists('charity_is_hope_sc_twitter')) {	
	function charity_is_hope_sc_twitter($atts, $content=null){	
		...
		$twitter_username = $user ? $user : charity_is_hope_get_theme_option('twitter_username');
		$twitter_consumer_key = $consumer_key ? $consumer_key : charity_is_hope_get_theme_option('twitter_consumer_key');
		$twitter_consumer_secret = $consumer_secret ? $consumer_secret : charity_is_hope_get_theme_option('twitter_consumer_secret');
		$twitter_token_key = $token_key ? $token_key : charity_is_hope_get_theme_option('twitter_token_key');
		$twitter_token_secret = $token_secret ? $token_secret : charity_is_hope_get_theme_option('twitter_token_secret');
		$twitter_count = max(1, $count ? $count : intval(charity_is_hope_get_theme_option('twitter_count')));
		...
		if (!empty($twitter_consumer_key) && !empty($twitter_consumer_secret) && !empty($twitter_token_key) && !empty($twitter_token_secret)) {
			...
			if ($data && isset($data[0]['text'])) {
				charity_is_hope_enqueue_slider('swiper');
				$output = ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || charity_is_hope_strlen($bg_texture)>2 || ($scheme && !charity_is_hope_param_is_off($scheme) && !charity_is_hope_param_is_inherit($scheme))
						? '<div class="sc_twitter_wrap sc_section'
								...
								. ($align && $align!='none' && !charity_is_hope_param_is_inherit($align) ? ' align' . esc_attr($align) : '')
								. '"'
							.' style="'
						...
						. '<div class="sc_twitter'
								...
								. ($bg_color=='' && $bg_image=='' && $bg_overlay==0 && ($bg_texture=='' || $bg_texture=='0') && $align && $align!='none' && !charity_is_hope_param_is_inherit($align) ? ' align' . esc_attr($align) : '')
								. '"'
							...
							$output .= '<div class="swiper-slide" data-style="'.esc_attr(($ws).($hs)).'" style="'.esc_attr(($ws).($hs)).'">'
										. '<div class="sc_twitter_item">'
											...
											. '<div class="sc_twitter_content">'
												...
												. force_balance_tags(charity_is_hope_prepare_twitter_text($tweet))
											. '</div>'
										. '</div>'
									. '</div>';
				...
				$output .= '</div>'
						...
						. '</div>'
					. '</div>'
					. ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || charity_is_hope_strlen($bg_texture)>2
						?  '</div></div>'
						: '');
			}
		}
		return apply_filters('charity_is_hope_shortcode_output', $output, 'trx_twitter', $atts, $content);
	}
	charity_is_hope_require_shortcode('trx_twitter', 'charity_is_hope_sc_twitter');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'charity_is_hope_sc_twitter_reg_shortcodes' ) ) {
	//add_action('charity_is_hope_action_shortcodes_list', 'charity_is_hope_sc_twitter_reg_shortcodes');
	function charity_is_hope_sc_twitter_reg_shortcodes() {
	
		charity_is_hope_sc_map("trx_twitter", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'charity_is_hope_sc_twitter_reg_shortcodes_vc' ) ) {
	//add_action('charity_is_hope_action_shortcodes_list_vc', 'charity_is_hope_sc_twitter_reg_shortcodes_vc');
	function charity_is_hope_sc_twitter_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Twitter extends CHARITY_IS_HOPE_VC_ShortCodeSingle {}
	}
}
?>