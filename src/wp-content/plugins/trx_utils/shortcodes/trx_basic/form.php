...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('charity_is_hope_sc_form_theme_setup')) {
	add_action( 'charity_is_hope_action_before_init_theme', 'charity_is_hope_sc_form_theme_setup' );
	function charity_is_hope_sc_form_theme_setup() {
		add_action('charity_is_hope_action_shortcodes_list', 		'charity_is_hope_sc_form_reg_shortcodes');
		if (function_exists('charity_is_hope_exists_visual_composer') && charity_is_hope_exists_visual_composer())
			add_action('charity_is_hope_action_shortcodes_list_vc','charity_is_hope_sc_form_reg_shortcodes_vc');
	}
}
...
if (!function_exists('charity_is_hope_sc_form')) {	
	function charity_is_hope_sc_form($atts, $content = null) {
		...
		charity_is_hope_enqueue_messages();	// Load core messages
	
		charity_is_hope_storage_set('sc_form_data', array(
			'id' => $id,
            'counter' => 0
            )
        );
		...
		$output = '<div ' . ($id ? ' id="'.esc_attr($id).'_wrap"' : '')
			...
			.'<div ' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_form'
					...
					. (!empty($align) && !charity_is_hope_param_is_off($align) ? ' align'.esc_attr($align) : '') 
				...
				. '>'
					. (!empty($subtitle) 
						? '<h6 class="sc_form_subtitle sc_item_subtitle">' . trim(charity_is_hope_strmacros($subtitle)) . '</h6>' 
						: '')
					. (!empty($title) 
						? '<h2 class="sc_form_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(charity_is_hope_strmacros($title)) . '</h2>' 
						: '')
					. (!empty($description) && $style != 'form_2'
						? '<div class="sc_form_descr sc_item_descr">' . trim(charity_is_hope_strmacros($description)) . ($style == 1 ? do_shortcode('[trx_socials size="tiny" shape="round"][/trx_socials]') : '') . '</div>' 
						: '');
		
		$output .= charity_is_hope_show_post_layout(array(
												'layout' => $style,
												'id' => $id,
												'action' => $action,
												'content' => $content,
		...
		return apply_filters('charity_is_hope_shortcode_output', $output, 'trx_form', $atts, $content);
	}
	charity_is_hope_require_shortcode("trx_form", "charity_is_hope_sc_form");
}

if (!function_exists('charity_is_hope_sc_form_item')) {	
	function charity_is_hope_sc_form_item($atts, $content=null) {
		if (charity_is_hope_in_shortcode_blogger()) return '';
		extract(charity_is_hope_html_decode(shortcode_atts( array(
			// Individual params
			"type" => "text",
		...
		charity_is_hope_storage_inc_array('sc_form_data', 'counter');
	
		$class .= ($class ? ' ' : '') . charity_is_hope_get_css_position_as_classes($top, $right, $bottom, $left);
		if (empty($id)) $id = charity_is_hope_storage_get_array('sc_form_data', 'id').'_'.charity_is_hope_storage_get_array('sc_form_data', 'counter');
		...
		if ($type == 'textarea')
			...
			wp_enqueue_script( 'jquery-picker', charity_is_hope_get_file_url('/js/picker/picker.js'), array('jquery'), null, true );
			wp_enqueue_script( 'jquery-picker-date', charity_is_hope_get_file_url('/js/picker/picker.date.js'), array('jquery'), null, true );
			...
			wp_enqueue_script( 'jquery-picker', charity_is_hope_get_file_url('/js/picker/picker.js'), array('jquery'), null, true );
			wp_enqueue_script( 'jquery-picker-time', charity_is_hope_get_file_url('/js/picker/picker.time.js'), array('jquery'), null, true );
		...
		return apply_filters('charity_is_hope_shortcode_output', $output, 'trx_form_item', $atts, $content);
	}
	charity_is_hope_require_shortcode('trx_form_item', 'charity_is_hope_sc_form_item');
}

// AJAX Callback: Send contact form data
if ( !function_exists( 'charity_is_hope_sc_form_send' ) ) {
	function charity_is_hope_sc_form_send() {
	
		if ( !wp_verify_nonce( charity_is_hope_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
			die();
	
		$response = array('error'=>'');
		if (!($contact_email = charity_is_hope_get_theme_option('contact_email')) && !($contact_email = charity_is_hope_get_theme_option('admin_email'))) 
			$response['error'] = esc_html__('Unknown admin email!', 'trx_utils');
		else {
			$type = charity_is_hope_substr($_REQUEST['type'], 0, 7);
			parse_str($_POST['data'], $post_data);

			if (in_array($type, array('form_1', 'form_2'))) {
				$user_name	= charity_is_hope_strshort($post_data['username'],	100);
				$user_email	= charity_is_hope_strshort($post_data['email'],	100);
				$user_subj	= charity_is_hope_strshort($post_data['subject'],	100);
				$user_msg	= charity_is_hope_strshort($post_data['message'],	charity_is_hope_get_theme_option('message_maxlength_contacts'));
            ...
            $mail = charity_is_hope_get_theme_option('mail_function') == 'mail' ? 'mail' : 'wp_mail';
            if (is_email($contact_email) && !@$mail($contact_email, $subj, apply_filters('charity_is_hope_filter_form_send_message', $msg))) {
...
if ( !function_exists( 'charity_is_hope_sc_form_show_fields' ) ) {
	function charity_is_hope_sc_form_show_fields($fields) {
...
-------------------------------------------------------------------- */
if ( !function_exists( 'charity_is_hope_sc_form_reg_shortcodes' ) ) {
	//add_action('charity_is_hope_action_shortcodes_list', 'charity_is_hope_sc_form_reg_shortcodes');
	function charity_is_hope_sc_form_reg_shortcodes() {
	
		$pages = charity_is_hope_get_list_pages(false);

		charity_is_hope_sc_map("trx_form", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"divider" => true,
					"value" => 'form_custom',
					"options" => charity_is_hope_get_sc_param('forms'),
					"type" => "checklist"
				), 
				"scheme" => array(
			...
			"children" => array(
				...
				"params" => array(
					"type" => array(
						...
						"type" => "checklist",
						"dir" => "horizontal",
						"options" => charity_is_hope_get_sc_param('field_types')
					), 
					"name" => array(
					...
					"label_position" => array(
						...
						"type" => "checklist",
						"dir" => "horizontal",
						"options" => charity_is_hope_get_sc_param('label_positions')
					), 
					"top" => charity_is_hope_get_sc_param('top'),
					"bottom" => charity_is_hope_get_sc_param('bottom'),
					"left" => charity_is_hope_get_sc_param('left'),
...
-------------------------------------------------------------------- */
if ( !function_exists( 'charity_is_hope_sc_form_reg_shortcodes_vc' ) ) {
	//add_action('charity_is_hope_action_shortcodes_list_vc', 'charity_is_hope_sc_form_reg_shortcodes_vc');
	function charity_is_hope_sc_form_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"std" => "form_custom",
					"value" => array_flip(charity_is_hope_get_sc_param('forms')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scheme",
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(charity_is_hope_get_sc_param('field_types')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "name",
				...
				array(
					...
					"class" => "",
					"value" => array_flip(charity_is_hope_get_sc_param('label_positions')),
					"type" => "dropdown"
				),
				charity_is_hope_get_vc_param('id'),
				charity_is_hope_get_vc_param('class'),
		...
		class WPBakeryShortCode_Trx_Form extends CHARITY_IS_HOPE_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Form_Item extends CHARITY_IS_HOPE_VC_ShortCodeItem {}
	}
}
?>