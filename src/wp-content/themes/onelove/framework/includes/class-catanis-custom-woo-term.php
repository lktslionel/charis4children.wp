<?php
 /**
 * ============================================================
 * Description: Adds some custom field for Product category
 *
...
if( !class_exists('Catanis_Custom_Woo_Term') ){
	class Catanis_Custom_Woo_Term {
		
		public static $_key_save_custom = 'cata_custom_woo_term';
		...
		function __construct()   {	
			...
			add_action( 'product_cat_add_form_fields', array( $this, 'add_form_fields' ), 20 );
			add_action( 'product_cat_edit_form_fields', array( $this, 'edit_form_fields' ), 10, 2 );
			add_action( 'edited_product_cat', array( $this, 'save_data' ), 10, 2 );
			add_action( 'create_product_cat', array( $this, 'save_data' ), 10, 2 );
			add_action( 'delete_product_cat', array( $this, 'delete_data' ), 10, 1 );
			
			$this->_default_data = array(
				'layout' 			=> 'default',
				'sidebar' 			=> 'shop_sidebar',
				'header_overlap' 	=> 'default',
				'breadcrumb_bg' 	=> '',
			);
		}
		...
		/*=== Add fields for Product Category in add layout ===*/
		...
		/*=== Add fields for Product Category in edit layout ===*/
