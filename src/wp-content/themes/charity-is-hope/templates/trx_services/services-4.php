...
if ( !function_exists( 'charity_is_hope_template_services_4_theme_setup' ) ) {
	add_action( 'charity_is_hope_action_before_init_theme', 'charity_is_hope_template_services_4_theme_setup', 1 );
	function charity_is_hope_template_services_4_theme_setup() {
		charity_is_hope_add_template(array(
			'layout' => 'services-4',
			'template' => 'services-4',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 4/', 'charity-is-hope')
		));
	}
}

// Template output
if ( !function_exists( 'charity_is_hope_template_services_4_output' ) ) {
	function charity_is_hope_template_services_4_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if (empty($post_data['post_icon'])) $post_data['post_icon']='icon-right';
					charity_is_hope_show_layout(charity_is_hope_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]'));
				?><span class="sc_services_item_title"><?php charity_is_hope_show_layout($post_data['post_title']); ?></span><?php
				if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
					?></a><?php
				}
				?>
