...
if ( !function_exists( 'charity_is_hope_template_gallery_theme_setup' ) ) {
	add_action( 'charity_is_hope_action_before_init_theme', 'charity_is_hope_template_gallery_theme_setup', 1 );
	function charity_is_hope_template_gallery_theme_setup() {
		charity_is_hope_add_template(array(
			'layout' => 'gallery_2',
			'template' => 'gallery',
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Gallery tile with preview mode /2 columns/', 'charity-is-hope'),
		...
		charity_is_hope_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Gallery tile /3 columns/', 'charity-is-hope'),
			'thumb_title'  => esc_html__('Medium image', 'charity-is-hope'),
			'w'		 => 370,
			'h_crop' => 209,
			'h'		 => null
		...
		charity_is_hope_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Gallery tile /4 columns/', 'charity-is-hope'),
			'thumb_title'  => esc_html__('Medium image', 'charity-is-hope'),
			'w'		 => 370,
			'h_crop' => 209,
			'h'		 => null
		...
		add_action('charity_is_hope_action_blog_scripts', 'charity_is_hope_template_gallery_add_scripts');
	}
}

// Add template specific scripts
if (!function_exists('charity_is_hope_template_gallery_add_scripts')) {
	//Handler of add_action('charity_is_hope_action_blog_scripts', 'charity_is_hope_template_gallery_add_scripts');
...
// Template output
if ( !function_exists( 'charity_is_hope_template_gallery_output' ) ) {
	function charity_is_hope_template_gallery_output($post_options, $post_data) {
		...
		$original_size = !empty($post_data['post_attachment']) ? charity_is_hope_getimagesize($post_data['post_attachment']) : array(0,0);
		?>
		<div class="isotope_item isotope_item_<?php echo esc_attr($style); ?> isotope_item_<?php echo esc_attr($post_options['layout']); ?> isotope_column_<?php echo esc_attr($columns); ?>
			...
			<<?php charity_is_hope_show_layout($tag); ?> class="post_item post_item_<?php echo esc_attr($style); ?> post_item_<?php echo esc_attr($post_options['layout']); ?>
				...
				<div class="post_content isotope_item_content">
					<div class="post_featured <?php echo (trim($post_data['post_type']) == 'matches' ? 'matches_hover' : '');?>"><?php
						...
						<div class="post_details">
							...
							?>
							<h2 class="post_title"><?php charity_is_hope_show_layout($post_data['post_title'], $link_start, $link_end); ?></h2>
							<div class="post_description">
								...
								if ($post_data['post_link'] != '') {
									...
									if ((!isset($post_options['readmore']) || !charity_is_hope_param_is_off($post_options['readmore'])) && !in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status'))) {
										?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_button sc_button_style_filled post_readmore"><span class="post_readmore_label"><?php echo !empty($post_options['readmore']) ? trim($post_options['readmore']) : esc_html__('Learn more', 'charity-is-hope'); ?></span></a><?php
									}
									?></p><?php
								}
								?>
			...
			</<?php charity_is_hope_show_layout($tag); ?>>
		</div>
		<?php
	}
}
