...
if ( !function_exists( 'charity_is_hope_template_testimonials_4_theme_setup' ) ) {
	add_action( 'charity_is_hope_action_before_init_theme', 'charity_is_hope_template_testimonials_4_theme_setup', 1 );
	function charity_is_hope_template_testimonials_4_theme_setup() {
		charity_is_hope_add_template(array(
			'layout' => 'testimonials-4',
			'template' => 'testimonials-4',
			'mode'   => 'testimonials',
			'title'  => esc_html__('Testimonials /Style 4/', 'charity-is-hope'),
			'thumb_title'  => esc_html__('Avatar (small)', 'charity-is-hope'),
			'w'		 => 75,
			'h'		 => 75
		));
	}
...
// Template output
if ( !function_exists( 'charity_is_hope_template_testimonials_4_output' ) ) {
	function charity_is_hope_template_testimonials_4_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo !empty($post_options['tag_css']) ? ' style="'.esc_attr($post_options['tag_css']).'"' : '';?>>
				<div class="sc_testimonial_content"><?php charity_is_hope_show_layout($post_data['post_content']); ?></div>
				<?php if ($post_options['photo']) { ?>
				<div class="sc_testimonial_avatar"><?php charity_is_hope_show_layout($post_options['photo']); ?></div>
				<?php } ?>
