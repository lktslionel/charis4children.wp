...
function charity_is_hope_theme_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = CHARITY_IS_HOPE_STORAGE['co_add_params']!=undefined ? CHARITY_IS_HOPE_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#charity_is_hope-customizer-styles-'+option).length > 0 ? jQuery('#charity_is_hope-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="charity_is_hope-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
