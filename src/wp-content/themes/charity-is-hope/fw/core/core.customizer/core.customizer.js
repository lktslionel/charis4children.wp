// Charity Is Hope Customizer script
...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.charity_is_hope_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.charity_is_hope_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && charity_is_hope_get_cookie('to_demo_fixed')==1) charity_is_hope_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.charity_is_hope_options_field_info .to_demo_pin', function(e) {
		charity_is_hope_options_fix_demo(jQuery(this).parents('.charity_is_hope_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	charity_is_hope_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.charity_is_hope_options_input,.charity_is_hope_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.charity_is_hope_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		charity_is_hope_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.charity_is_hope_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.charity_is_hope_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .charity_is_hope_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.charity_is_hope_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? CHARITY_IS_HOPE_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.charity_is_hope_options_tab').find('> .charity_is_hope_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="charity_is_hope_options_schemes_order" class="charity_is_hope_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function charity_is_hope_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.charity_is_hope_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.charity_is_hope_options').find('>.charity_is_hope_options_field_info');
...
function charity_is_hope_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.charity_is_hope_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	charity_is_hope_message_confirm(CHARITY_IS_HOPE_STORAGE['to_strings']['scheme_delete_confirm'], CHARITY_IS_HOPE_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'charity_is_hope_options_scheme_delete',
			nonce: CHARITY_IS_HOPE_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.charity_is_hope_options_tab').find('> .charity_is_hope_options_schemes_order').val()
		};
		setTimeout(function(){
			charity_is_hope_message_info(CHARITY_IS_HOPE_STORAGE['to_strings']['recompile_styles'], CHARITY_IS_HOPE_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(CHARITY_IS_HOPE_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				charity_is_hope_message_success(CHARITY_IS_HOPE_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+CHARITY_IS_HOPE_STORAGE['to_strings']['reload_page'], CHARITY_IS_HOPE_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				charity_is_hope_message_warning(CHARITY_IS_HOPE_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, CHARITY_IS_HOPE_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function charity_is_hope_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.charity_is_hope_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	charity_is_hope_message_confirm(CHARITY_IS_HOPE_STORAGE['to_strings']['scheme_copy_confirm'], CHARITY_IS_HOPE_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'charity_is_hope_options_scheme_copy',
			nonce: CHARITY_IS_HOPE_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.charity_is_hope_options_tab').find('> .charity_is_hope_options_schemes_order').val()
		};
		...
		jQuery.post(CHARITY_IS_HOPE_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				charity_is_hope_message_success(CHARITY_IS_HOPE_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+CHARITY_IS_HOPE_STORAGE['to_strings']['reload_page'], CHARITY_IS_HOPE_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				charity_is_hope_message_warning(CHARITY_IS_HOPE_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, CHARITY_IS_HOPE_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function charity_is_hope_options_fix_demo(info) {
	var info2 = info.parents('.charity_is_hope_options').toggleClass('to_demo_fixed').find('>.charity_is_hope_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.charity_is_hope_options').append(clonedObj);
		info2 = info.parents('.charity_is_hope_options').find('>.charity_is_hope_options_field_info');
	}
	charity_is_hope_set_cookie('to_demo_fixed', jQuery('.charity_is_hope_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function charity_is_hope_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.charity_is_hope_options_field_info').data('scheme');
