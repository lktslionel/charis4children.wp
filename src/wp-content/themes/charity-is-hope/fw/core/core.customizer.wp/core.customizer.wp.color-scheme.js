/* global charity_is_hope_color_schemes, charity_is_hope_dependencies, Color */
...
( function( api ) {
	...
	if (!charity_is_hope_customizer_vars['need_refresh']) {
		for (var i in charity_is_hope_color_schemes) {
			cssTemplate[i] = wp.template( 'charity_is_hope-color-scheme-'+i );
		}
		cssTemplate['fonts'] = wp.template( 'charity_is_hope-fonts' );
	}
	...
	api.bind('ready', function() {
		...
		// Add 'Refresh' button
		if (charity_is_hope_customizer_vars['need_refresh']) {
			jQuery('#customize-header-actions .spinner').after('<input type="button" class="button customize-action-refresh" value="'+charity_is_hope_customizer_vars['msg_refresh']+'">');
		...
		jQuery('#customize-theme-controls .control-section').each(function () {
			charity_is_hope_customizer_wp_check_dependencies(jQuery(this));
		});

		// Update colors
		charity_is_hope_customizer_wp_change_color_scheme(api('color_scheme')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		charity_is_hope_customizer_wp_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		charity_is_hope_customizer_wp_refresh_preview(obj);
	});

	// Check for dependencies
	function charity_is_hope_customizer_wp_check_dependencies(cont) {
		"use strict";
		...
		cont.find('.customize-control').each(function() {
			...
			var depend = false;
			for (var fld in charity_is_hope_dependencies) {
				if (fld == id) {
					depend = charity_is_hope_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('charity_is_hope_options_no_use');
				} else {
					jQuery(this).hide().addClass('charity_is_hope_options_no_use');
				}
			}
		});
	}
	...
	function charity_is_hope_customizer_wp_refresh_preview(obj) {
		"use strict";
		...
		if (id == 'color_scheme') {
			charity_is_hope_customizer_wp_change_color_scheme(val);
		} else if (updateCSS) {
			var simple = api('color_settings')()=='simple';
			for (var opt in charity_is_hope_color_schemes['original']) {
				if (opt == id) {
					updateCSS = false;
					// Store new value in the color table
					charity_is_hope_customizer_wp_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							charity_is_hope_customizer_wp_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							charity_is_hope_customizer_wp_update_color_scheme('alter_hover', val);
						}
					}
					updateCSS = true;
					processed = true;
			...
			if (!processed) {
				for (var tag in charity_is_hope_fonts) {
					for (var prop in charity_is_hope_fonts[tag]) {
						if (prop=='title' || prop=='description') continue;
						if (tag+'-'+prop == id) {
							processed = true;
							// Store new value in the fonts table
							charity_is_hope_customizer_wp_update_fonts(tag, prop, val);
							break;
						}
					}
					if (processed) break;
			...
			// Refresh CSS
			if (processed) charity_is_hope_customizer_wp_update_css();
		}
	...
	// Store new value in the color table
	function charity_is_hope_customizer_wp_update_color_scheme(opt, value) {
		"use strict";
		charity_is_hope_color_schemes[api('color_scheme')()][opt] = value;
		api('scheme_storage').set(charity_is_hope_serialize(charity_is_hope_color_schemes))
	}
	

	// Change color scheme - update colors and generate css
	function charity_is_hope_customizer_wp_change_color_scheme(value) {
		"use strict";
		updateCSS = false;
		for (var opt in charity_is_hope_color_schemes[value]) {
			if (api(opt) == undefined) continue;
			api( opt ).set( charity_is_hope_color_schemes[value][opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', charity_is_hope_color_schemes[value][opt] )
				.wpColorPicker( 'defaultColor', charity_is_hope_color_schemes[value][opt] );
		}
		updateCSS = true;
		charity_is_hope_customizer_wp_update_css();
	}

	// Store new value in the fonts table
	function charity_is_hope_customizer_wp_update_fonts(tag, prop, value) {
		"use strict";
		charity_is_hope_fonts[tag][prop] = value;
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function charity_is_hope_customizer_wp_update_css() {
		"use strict";

		if (!updateCSS || charity_is_hope_customizer_vars['need_refresh']) return;

		var css = '';
		
		// Add color styles
		for (var scheme in charity_is_hope_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in charity_is_hope_color_schemes[scheme]) {
				if (i=='title') continue;
				colors[i] = charity_is_hope_color_schemes[scheme][i];
			}
		...
		var fonts = [], font = [];
		for (var tag in charity_is_hope_fonts) {
			font = charity_is_hope_fonts[tag];
		...
		// Make theme specific colors and tints
		if (window.charity_is_hope_customizer_wp_add_theme_fonts) fonts = charity_is_hope_customizer_wp_add_theme_fonts(fonts);
	...
	// Add ed to css value
	function charity_is_hope_prepare_css_value(val) {
		"use strict";
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
	...
	// Multiply css value
	function charity_is_hope_multiply_css_value(val, mult) {
		"use strict";
		var reg = /([\d\.]+)([a-zA-Z%]*)/i;
		if (val != '' && val != 'inherit') {
	...
	// Summ css value
	function charity_is_hope_summ_css_value(val1, val2) {
		"use strict";
		var reg = /([\d\.]+)([a-zA-Z%]*)/i;
